...
from trytond.pyson import Eval, Bool, If, PYSON
...
ZERO = Decimal(0)
PARTY_SIMILARITY_THRESHOLD = config.get('account_statement_common',
    'party_similarity_threshold', default=config.get('enable_banking',
        ...
        if any(line.move for line in lines):
            ...
            if Warning.check(warning_name):
                raise StatementValidateWarning(warning_name,
                    gettext('account_statement_common.'
...
class Origin(Workflow, metaclass=PoolMeta):
    ...
    def on_change_with_synchronized(self, name=None):
        "Return whether the source connector protects the bank transaction."
        return False
    ...
    @ModelView.button_action(
            'account_statement_common.wizard_multiple_invoices')
    def multiple_invoices(cls, origins):
        pass

    @classmethod
    @ModelView.button_action(
            'account_statement_common.wizard_multiple_move_lines')
    def multiple_move_lines(cls, origins):
        pass

    @classmethod
    @ModelView.button_action(
            'account_statement_common.wizard_link_invoice')
    ...
    def find_same_related_origin(cls, origins):
        ...
        for related_to, values in relateds_to.items():
            if ((values['amount'] >= 0 and values['diff'] < 0)
                ...
                raise AccessError(gettext('account_statement_common.'
...
class OriginCreateStamentLine(Wizard):
    ...
    start = StateView(
        ...
        'account_statement_common.create_line_start_view_form', [
            Button('Cancel', 'end', 'tryton-cancel'),
            Button('OK', 'create_lines', 'tryton-ok', True),
            ])
